...
def pack_binary_frame(msg_type, metadata: dict, image_bytes: bytes):
    """打包 binary frame: [Header(16)][Metadata JSON][Image] Header: - Magic (4 bytes, little-endian uint32) = 0x5959 -
    Version (1 byte) = 1 - MessageType (1 byte) - Reserved (2 bytes) = 0 - MetadataLength (4 bytes, big-endian) -
...
def parse_binary_request(data: bytes):
    """解析 binary request frame。 Returns: (msg_type, device_id, device_name, metadata, image_bytes) or (None, None, None,
    None, None) on error.
    """
    if len(data) < 16:
        return None, None, None, None, None

    magic, _version, msg_type = struct.unpack_from("<IBB", data, 0)
...
async def broadcast_client_list_to_controllers():
    ...
    await asyncio.gather(*[ws.send(payload) for ws in control_clients.values()], return_exceptions=True)
    ...
    try:
        ...
        def ensure_session(device_id: str, device_name: str):
            ...
            if connection_id not in clients:
                ...
                signal_emitter.client_connected.emit(
                    connection_id, device_id, device_name, session_obj.ip, session_obj.port
                )
        ...
        async for message in websocket:
            ...
            # ============================================================
                ...
                if message_type == "detect":
                    ...
                    cmd = json.dumps({"type": "query", "query": query}, ensure_ascii=False)
                    if target_id in clients:
                        await send_to_client(target_id, cmd)
