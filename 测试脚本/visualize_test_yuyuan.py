...
def visualize_single_image(model, image_path, conf_threshold=0.25):
    ...
    ax1.imshow(img_rgb)
    ax1.set_title(
        f"检测结果 - {Path(image_path).name}\n检测到 {len(result.boxes)} 个目标", fontsize=14, fontweight="bold"
    )
    ax1.axis("off")
    ...
    for box in result.boxes:
        ...
        text_bbox = {"boxstyle": "round,pad=0.5", "facecolor": color, "alpha": 0.8}
        ax1.text(x1, y1 - 10, label, fontsize=12, color="white", fontweight="bold", bbox=text_bbox)
...
def visualize_multiple_images(model, image_folder, num_images=6, conf_threshold=0.25):
    ...
    for idx, image_path in enumerate(image_files):
        ...
        ax.imshow(img_rgb)
        ax.set_title(f"{image_path.name}\n检测: {len(result.boxes)} 个目标", fontsize=10, fontweight="bold")
        ax.axis("off")
